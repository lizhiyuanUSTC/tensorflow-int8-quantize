...
import cfg
from utils import int_quantize, _variable_on_cpu, _variable_with_weight_decay
...
def conv_bn_relu(x, out_channels, ksize, stride=1, groups=1,
...
def add(x, y, phase_train, has_relu=False, scale=None, qactivation=False, scope=None):
    f = x + y
    if has_relu:
        f = tf.nn.relu6(f)
    node = {'input': [x, y],
